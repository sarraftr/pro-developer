...
Fatura kesilmeden önce kalemleri, fatura türünü ve toplamları hesaplar. **E-belge servisine gönderim yapılmaz, fatura kaydı oluşmaz.** Gövde `POST /v1/invoices` ile aynıdır; yalnız `isDraft` ve `allowDuplicate` alanları kullanılamaz.
...
Taslak (`isDraft: true`) isteklerinde HTTP **200** döner: belge serviside taslak oluşur, resmî fatura kesilmez, `invoiceId` ve `totals` boş gelir.
