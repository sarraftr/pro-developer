---
title: "Gider pusulası işlemleri"
description: "Kalem şablonu seçin, tutarı hesaplatın, pusulayı oluşturup kesinleştirin, taslağı iptal edin"
---
...
|---|---|
| `bankTransaction` | Aktif banka hesabına bağlı, resmileştirilmemiş bir **giden** banka hareketi için pusula kesilir. Hareket resmileştirilmiş olarak işaretlenir. |
| `manual` | Banka hareketine bağlı olmayan **serbest** gider pusulası. |

<Note>
  Banka hareketi geçmişte okunabiliyor olsa bile bağlı hesap pasifse hareket gider pusulası kaynağı olarak kullanılamaz. Önizleme, taslak ve oluşturma `409 BANK_ACCOUNT_INACTIVE` döndürür. Hesabı yeniden aktifleştirdiğinizde işlenmemiş hareket tekrar kullanılabilir.
</Note>
...
Miktar, birim fiyat, mahiyet ve müşteri bilgisini hesaplar. **Kayıt oluşmaz, seri-sıra numarası tüketilmez.** `source: "bankTransaction"` kullanıyorsanız bağlı banka hesabı aktif olmalıdır. Gövde `POST /v1/expenses` ile aynıdır; `isDraft` ve `allowDuplicate` kullanılamaz.
...
| Banka hareketinin bağlı hesabı pasif | 409 | `BANK_ACCOUNT_INACTIVE` — önizleme, taslak ve oluşturma reddedilir |
